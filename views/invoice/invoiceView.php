             	...
             	<form class="form-horizontal"  onsubmit="return false;">
					<div class="form-group">
					  ...
					  <div class="col-sm-9">
						...
						<input type="text" readonly name="project_name" value="<?php echo $inv->{'project_name'};?>" class="form-control">
					  </div>
					</div>
					<div class="form-group">
					  <label class="col-sm-3 control-label">Early Redemption</label>
					  <div class="col-sm-9">
					  <select class="form-control" name="early_redemption">
					...
					<div class="form-group">
					  ...
					  <div class="col-sm-9">
					  	<?php
					  	// echo isset($inv->{'inv_starter_pack'}) && $inv->{'inv_starter_pack'} == 1 ? 'Yes' : 'No';
					  	if ($agreement_type == 1)
					  		echo 'Standard';
					...
					$t['inv_id'] = $inv->{'inv_id'};
					$t['user_type'] = 1;
					$m_transfer = $this->Invoice_model->getInvoiceTransfer($t);
						
					$t['inv_id'] = $inv->{'inv_id'};
					$t['user_type'] = 2;
					$a_transfer = $this->Invoice_model->getInvoiceTransfer($t);
					?>
					...
					<div class="form-group">
					  ...
					  <div class="col-sm-9">
					  	<?php
					  		$amt = $inv->{'inv_amt'};
// 					  	if (count($a_transfer) > 0 || count($m_transfer) > 0) 
// 					  		$amt = 0;
					  		?>
					  	
					  	
						<input type="text" readonly id="inv_amt" name="inv_amt" placeholder="number only"  value="<?php echo $amt;?>" class="form-control">
 ...
 -->
					...
					<div class="form-group">
					  ...
					  <div class="col-sm-9">
					 		...
					 		<?php echo isset($inv->{'dr_name'}) ? $inv->{'dr_name'} : '';?>
					...
					if (count($m_transfer) > 0) {
						?>
						<div class="form-group">
						  <label class="col-sm-3 control-label">Transfer from Manager:</label>
						  <div class="col-sm-9">
								<?php echo $this->Manager_model->getManagerById($m_transfer->{'from_user'})->{"m_name"};?>
					...
					if (count($a_transfer) > 0) {
						?>
						<div class="form-group">
						  <label class="col-sm-3 control-label">Transfer from Agent:</label>
						  <div class="col-sm-9">
							<?php echo $this->Agent_model->getAgentById($a_transfer->{'from_user'})->{"agent_name"};?>
					...
					<div class="form-group">
					  ...
					  <div class="col-sm-9">
							<?php echo isset($inv->{'aagent'}->{"agent_name"}) ? $inv->{'aagent'}->{"agent_name"} : '-';?>
					...
					<div class="form-group">
					  ...
					  <div class="col-sm-9">
						...
						<input type="text" readonly id="cheque_no3" name="cheque_no3"  class="form-control" value="<?php echo set_value('cheque_no3', $inv->{'cheque_no3'});?>"/>
					  </div>
					</div>
					...
					<div class="form-group">
					  ...
					  <div class="col-sm-9">
						...
						if (file_exists(PAYOUT_PATH.$inv->{"inv_id"}.'.pdf')) 
							echo '<a href="'.PAYOUT_URL.$inv->{"inv_id"}.'.pdf" target="_blank">Customer Payout Schedule</a>';
...
// 							echo '-';
					...
					<div class="form-group">
					  ...
					  <div class="col-sm-9">
					 	 <?php
					  if (file_exists(RECEIPT_PATH.$inv->{"inv_id"}.'.pdf')) 
							echo '<a href="'.RECEIPT_URL.$inv->{"inv_id"}.'.pdf" target="_blank">Funding Receipt</a>';
...
// 							echo '-';
					...
					<div class="form-group">
					  ...
					  <div class="col-sm-9">
						...
						if (file_exists(RISK_PATH.$inv->{"inv_id"}.'.pdf')) 
							echo '<a href="'.RISK_URL.$inv->{"inv_id"}.'.pdf" target="_blank">Risk Accessment</a>';
// 							echo '<a href="'.base_url().'invoice/generateRisk/?id='.$inv->{'inv_id'}.'" target="_blank">Risk Assesment</a>';
						else
							echo '-';
              	...
              	<div class="porlets-content">
              		 ...
              		 <div class="tab-container">
             		 	<ul class="nav nav-tabs">
               				 <li class="active"><a href="#home" data-toggle="tab">Funder Payout</a></li>
               				
               				 <?php if ($this->session->userdata('user_type') == 'admin') {?>
               				 <li class=""><a href="#profile" data-toggle="tab">Sales Payout</a></li>
               				 
               				 <?php
               				 if (isset($additional_sales_payout) && count($additional_sales_payout) > 0) {
               				 	?>
               				 	<li class=""><a href="#profile2" data-toggle="tab">Second Agent Sales Payout</a></li>
               				 	<?php
               				 }
               				 ?>
               				  <?php }?>
										...
										if (($year == 1 && ($month <= 3) && $inv->{'inv_starter_pack'} == 0) && $inv->{'agreement_type'} != 2) {
											...
											if ($i == count($client_payout)) {
												echo '<td>$'.number_format($client_payout[$i-1]->{'amt'},2,'.',',').'<br/>'.
													'$'.number_format($inv->{'inv_total'},2,'.',',').'</td>';
												$total_per_year += $inv->{'inv_total'};
											}
											else
...
// 											else
												...
												$amt = count($a_transfer) > 0 ? 0 : $sales_payout[$i-1]->{'amt'};
...
// 										}
							...
							if (isset($additional_sales_payout) && count($additional_sales_payout) > 0) {
							?>
							<div class="tab-pane cont" id="profile2">
							 	<div class="table-responsive">
								<div class="alert alert-info"><b>Staff Comm: <?php echo $comm;?><br/>Staff Pipeline: <?php echo $pipeline;?></b></div>
								<?php
								if (isset($additional_sales_payout) && count($additional_sales_payout) > 0) {
									...
									for ($i =1; $i<= count($additional_sales_payout); $i++) {
										if ($month == 1) {
											?>
											<div class="alert alert-info"><b>YEAR <?php echo $year;?></b></div>
											<table class="table table-bordered">
										...
										echo '<td>'.date('Y-m-d', strtotime($additional_sales_payout[$i-1]->{'date'})).'</td>';
...
// 											else
												...
												echo '<td>$'.number_format($additional_sales_payout[$i-1]->{'amt'},2,'.',',').'</td>';
											$total_per_year += $additional_sales_payout[$i-1]->{'amt'};
// 										}		
...
// 										else {
											if ($this->session->userdata('user_type') != 'admin') {
												...
												if ($additional_sales_payout[$i-1]->{'pay_date'} == null) {
													echo '<td><a href="#">UNPAID</a></td>';
												}
												else {
													echo '<td><a class="paid-link" href="#">PAID</a></td>';
													...
													//havent pay
												if ($additional_sales_payout[$i-1]->{'pay_date'} == null) {
													echo '<td><a href="'.base_url().'invoice/markSalesPaid/?inv='.$_GET['inv'].'&id='.$additional_sales_payout[$i-1]->{'id'}.'">Pay</a></td>';
												}
												else {
													echo '<td><a class="paid-link" href="'.base_url().'invoice/markSalesUnpaid/?inv='.$_GET['inv'].'&id='.$additional_sales_payout[$i-1]->{'id'}.'">Paid</a></td>';
												}
											}
...
// 										}
										...
										//last row
										if ($month == 12 || $i == count($additional_sales_payout)) {
										?>	
											<tr>
												<td colspan="2" style="text-align:right"><b>Total Receivable:</b></td>
												<td colspan="2"><b>$<?php echo number_format($total_per_year, 2,'.',',');?></b></td>
...
// 											}
// 											else
												$amt = count($m_transfer) > 0 ? 0 : $manager_payout[$i-1]->{'amt'};
												echo '<td>$'.number_format($amt,2,'.',',').'</td>';
											$total_per_year += $amt;
// 										}		
