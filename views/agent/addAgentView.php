        ...
        <div class="col-md-9">
          <div class="block-web">
            ...
            <div class="porlets-content">
              ...
              ?>
             	<form class="form-horizontal" method="post" action="<?php echo base_url();?>agent/addAgent">
					...
					<div class="form-group">
					  ...
					  <div class="col-sm-9">
					  	 <?php echo form_error('agent_pass');?>
						<input type="password" name="agent_pass" value="<?php echo set_value('agent_pass', $agent_pass);?>" autocomplete="off" class="form-control">
					  </div>
					  </div>
						...
						<select name="agent_upline">
							<?php if (isset($managers) && count($managers) > 0) {
								foreach ($managers as $m) {
									echo '<option value="'.$m->{'m_id'}.'">'.$m->{'m_name'}.' ('.$m->{'m_code'}.')</option>';
								}
							}
							?>
						</select>
					  ...
					  <?php } else if ($this->session->userdata('user_type') == 'manager') {?>
					  	<input type="hidden" name="agent_upline" value="<?php echo $this->session->userdata('manager')->{'m_id'};?>"/>
					  <?php }?>
					  <div class="form-group">
					  <label class="col-sm-3 control-label">Agent code</label>
					  <div class="col-sm-9">
					  	 <?php echo form_error('agent_code');?>
						<input type="text" name="agent_code" value="<?php echo set_value('agent_code', $agent_code);?>" autocomplete="off" class="form-control">
					  </div>
					  </div>
